...
public class LogicalOr extends BinaryOperation {
    ...
    public LogicalOr(Expression e1, Expression e2) {
        super(e1, e2);
    }
    ...
    public Value eval(Environment env) throws VariableNotFoundException, TypeUnmatchedException {
        ...
        return VBoolean.fromBoolean(mExpr1.eval(env).asBoolean() || mExpr2.eval(env).asBoolean());
    }
    ...
    public CompiledCode compile(CompileEnvironment env, CompiledCode next)
        ...
        return mExpr1.compile(env, insn(OpCode.LORS, e2Compiled, next));
    }
